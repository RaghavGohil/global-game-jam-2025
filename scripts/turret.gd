...
func _process(delta):
	...
	# Rotate turret head toward the target
	if target:
		var direction = - (target.global_position - turret_head.global_position).normalized()
		
		# Compensate for parent rotation
		turret_head.rotation = direction.angle() - global_rotation - PI / 2
